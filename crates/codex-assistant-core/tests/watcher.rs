...
fn watcher_install_plan_registers_rust_launcher_at_logon() {
    let plan = build_watcher_install_plan("C:/Tools/codex-assistant.exe".into(), 9333);

    assert_eq!(plan.run_value_name, "CodexAssistantWatcher");
    assert_eq!(
        plan.run_value,
        "\"C:/Tools/codex-assistant.exe\" --debug-port 9333"
    );
    assert_eq!(plan.shortcut_name, "CodexAssistantWatcher.lnk");
    assert_eq!(plan.shortcut_target, "C:/Tools/codex-assistant.exe");
    assert_eq!(plan.shortcut_arguments, "--debug-port 9333");
}

#[test]
fn spawn_launcher_command_points_to_silent_binary_only() {
    let command = build_spawn_launcher_command("C:/Tools/codex-assistant.exe", 9444);

    assert_eq!(command[0], "C:/Tools/codex-assistant.exe");
...
fn launcher_process_filter_protects_current_process_ancestry() {
    let processes = [
        (10, 0, "codex-assistant.exe"),
        (20, 10, "codex-assistant.exe"),
        (30, 20, "codex-assistant.exe"),
        (40, 10, "codex-assistant.exe"),
        (50, 10, "codex-assistant-manager.exe"),
    ];
