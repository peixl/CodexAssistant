...
macro_rules! cdp_loopback_test {
    ($body:block) => {{
        if !async_loopback_available().await {
            eprintln!("skipping CDP bridge test because async 127.0.0.1 TCP is unavailable");
            return;
        }
        $body
    }};
...
fn injection_script_fetches_ads_without_bridge() {
    ...
    assert!(script.contains("peixl/Ad-List"));
    assert!(
...
async fn install_bridge_routes_binding_while_waiting_for_command_response() {
    cdp_loopback_test!({
        ...
        codex_assistant_core::diagnostic_log::set_diagnostic_log_path_for_tests(Some(
            log_path.clone(),
        ));
...
async fn install_bridge_does_not_wait_for_resolve_runtime_evaluate_ack() {
    cdp_loopback_test!({
        let (url, request_rx) = spawn_cdp_server(|mut socket| async move {
            ...
            .await;
            let second_resolve = tokio::time::timeout(
                Duration::from_millis(500),
                recv_json(&mut socket),
            )
            .await
            .expect("second resolve should be sent without waiting for first Runtime.evaluate ack");
...
async fn async_loopback_available() -> bool {
    ...
    let client = async {
        ...
        (buffer == *b"ok").then_some(())
    };
    let available = matches!(
        tokio::time::timeout(Duration::from_millis(500), client).await,
        Ok(Some(()))
    );
    server.abort();
    available
