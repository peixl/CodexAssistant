...
pub const SILENT_NAME: &str = "CodexAssistant";
pub const MANAGER_NAME: &str = "CodexAssistant 管理工具";
pub const SILENT_BINARY: &str = "codex-assistant";
pub const MANAGER_BINARY: &str = "codex-assistant-manager";
...
fn platform_install(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持安装 CodexAssistant 入口")
    }
}
...
fn platform_uninstall(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    {
        let _ = options;
        anyhow::bail!("当前平台暂不支持卸载 CodexAssistant 入口")
    }
}
