...
const RELAY_PROVIDER: &str = "CodexAssistant";
...
pub async fn test_relay_profile(
    ...
    let client = crate::http_client::proxied_client("CodexAssistant/RelayTest")?;
...
pub fn clear_relay_config_to_home(home: &Path) -> anyhow::Result<RelayApplyResult> {
    ...
    let config_path = home.join("config.toml");
    let existing = std::fs::read_to_string(&config_path).unwrap_or_default();
    let mut without_relay = remove_table(&existing, &format!("model_providers.{RELAY_PROVIDER}"));
    for legacy in LEGACY_RELAY_PROVIDERS {
        without_relay = remove_table(&without_relay, &format!("model_providers.{legacy}"));
    }
    without_relay = remove_root_key(&without_relay, "OPENAI_API_KEY");
...
fn upsert_model_provider_config(contents: &str, base_url: &str, bearer_token: &str) -> String {
    ...
    for legacy in LEGACY_RELAY_PROVIDERS {
        updated = remove_table(&updated, &format!("model_providers.{legacy}"));
    }
