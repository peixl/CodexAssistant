...
fn create_backup(
    ...
    fs::write(
        ...
        serde_json::to_string_pretty(
            &json!({"managedBy": "CodexAssistant provider sync", "targetProvider": target_provider}),
        )?,
    )?;
    Ok(backup_dir)
}
...
fn prune_backups(home: &Path) -> anyhow::Result<()> {
    ...
    for entry in fs::read_dir(&root)? {
        ...
        if value.get("managedBy").and_then(Value::as_str) == Some("CodexAssistant provider sync") {
            managed.push(path);
        }
    }
