...
fn github_payload_selects_platform_installer() {
    let release = release_from_github_payload(&json!({
        "tag_name": "v1.0.9",
        "html_url": "https://github.com/peixl/CodexAssistant/releases/tag/v1.0.9",
        "body": "fixes",
        "assets": [
    ...
    if cfg!(windows) {
        assert_eq!(
            ...
            Some("CodexAssistant_1.0.9_x64-setup.exe")
        );
    } else if cfg!(target_os = "macos") {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("CodexAssistant_1.0.9_x64.dmg")
        );
    } else {
...
fn latest_json_payload_selects_platform_installer_without_github_api_shape() {
    let release = release_from_latest_json_payload(&json!({
        "version": "v1.1.6",
        "url": "https://github.com/peixl/CodexAssistant/releases/tag/v1.1.6",
        "body": "静态更新描述",
        "assets": [
            {"name": "source.zip", "url": "https://example.test/source.zip"},
            {"name": "CodexAssistant-1.1.6-windows-x64-setup.exe", "url": "https://example.test/setup.exe"},
            {"name": "CodexAssistant-1.1.6-macos-x64.dmg", "url": "https://example.test/app.dmg"}
        ]
    }))
    .unwrap();
    ...
    if cfg!(windows) {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("CodexAssistant-1.1.6-windows-x64-setup.exe")
        );
    } else if cfg!(target_os = "macos") {
        assert_eq!(
            release.asset_name.as_deref(),
            Some("CodexAssistant-1.1.6-macos-x64.dmg")
        );
    } else {
        assert_eq!(release.asset_name.as_deref(), None);
    }
...
fn asset_selection_prefers_current_platform_artifacts() {
    ...
    if cfg!(windows) {
        ...
        assert_eq!(selected.name, "CodexAssistant_1.0.9_x64-setup.exe");
    } else if cfg!(target_os = "macos") {
        let selected = select_update_asset(&assets).unwrap();
        assert_eq!(selected.name, "CodexAssistant_1.0.9_x64.dmg");
    } else {
