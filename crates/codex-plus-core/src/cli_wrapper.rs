...
const CLI_HOME_DIR: &str = ".codex-assistant-cli";
...
pub fn install_cli_wrapper_to(
    ...
    std::fs::create_dir_all(codex_home).with_context(|| {
        format!(
            "failed to create CodexAssistant CLI home {}",
            codex_home.display()
        )
    })?;
...
pub fn wrapper_dir_from_roaming(roaming: &Path) -> PathBuf {
    #[cfg(not(windows))]
    {
        let roaming = roaming.as_os_str().to_string_lossy();
        if roaming.contains('\\') && !roaming.contains('/') {
            return PathBuf::from(format!(r"{roaming}\CodexAssistant"));
        }
    }
    roaming.join("CodexAssistant")
}
...
fn compile_wrapper(source_path: &Path, wrapper_path: &Path) -> anyhow::Result<()> {
    let csc = find_csc()
        .ok_or_else(|| anyhow::anyhow!("未找到 csc.exe，无法编译 CodexAssistant wrapper"))?;
