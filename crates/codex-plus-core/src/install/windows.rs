...
const UNINSTALL_SUBKEY: &str =
    r"Software\Microsoft\Windows\CurrentVersion\Uninstall\CodexAssistant";
#[cfg(windows)]
...
pub fn build_windows_entrypoint_plan(options: &InstallOptions) -> WindowsEntrypointPlan {
    ...
    WindowsEntrypointPlan {
        silent_shortcut: install_root
            .join("CodexAssistant.lnk")
            .to_string_lossy()
            .to_string(),
        manager_shortcut: install_root
            .join("CodexAssistant 管理工具.lnk")
            .to_string_lossy()
            .to_string(),
        ...
        uninstall_key: "CodexAssistant".to_string(),
...
fn write_uninstall_registration(plan: &WindowsEntrypointPlan) -> anyhow::Result<()> {
    ...
    let install_location = Path::new(&plan.manager_path)
        ...
        .to_string();
    for (name, value) in [
        ("DisplayName", "CodexAssistant".to_string()),
        ("DisplayVersion", crate::version::VERSION.to_string()),
        ("Publisher", "IFQ.AI".to_string()),
