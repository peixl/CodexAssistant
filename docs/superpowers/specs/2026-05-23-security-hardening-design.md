...
- `crates/codex-assistant-core/src/assets.rs::injection_script`：签名改为 `injection_script(helper_port: u16, helper_token: &str) -> String`，注入 `window.__CODEX_PLUS_HELPER_TOKEN__`。
- 调用链跟随：
