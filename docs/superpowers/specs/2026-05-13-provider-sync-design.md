...
Add an optional CodexAssistant startup sync that keeps existing Codex conversations visible after the user changes `model_provider`.

## User experience

CodexAssistant settings gains a `Provider 同步` toggle. The toggle is off by default.

When the toggle is off, CodexAssistant launch behavior is unchanged.

When the toggle is on, CodexAssistant runs provider sync before launching Codex. The sync targets the current root `model_provider` from `~/.codex/config.toml`; it does not switch providers or edit `config.toml`.

If sync is skipped because another sync is locked, SQLite is busy, or session files are in use, CodexAssistant writes the reason to `~/.codex-session-delete/launcher.log` and continues launching Codex. If sync starts writing and then cannot safely restore or finish, launch fails and the error is written to the launcher log.

## Architecture

Implement the feature inside CodexAssistant as Python code. Do not depend on the external Node.js CLI from `Dailin521/codex-provider-sync`, because CodexAssistant should not require Node 24 or a separate global package.

Add three focused areas:

- `codex_session_delete/settings_store.py` stores backend-readable CodexAssistant settings in `~/.codex-session-delete/settings.json`.
...
Initial content is implicit; if the file is missing or malformed, CodexAssistant uses defaults:
...
1. Turn on `Provider 同步` in the CodexAssistant menu.
2. Close Codex.
