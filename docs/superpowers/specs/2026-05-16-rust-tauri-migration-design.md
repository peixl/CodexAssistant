# CodexAssistant Rust/Tauri Migration Design

## Summary

CodexAssistant will fully replace the current Python backend with Rust and add a Tauri management console. The existing Codex App enhancement model remains: CodexAssistant launches Codex with CDP flags, injects a bridge plus `renderer-inject.js`, and handles local operations such as delete, undo, export, move, settings, status, provider sync, and user scripts.

The final user experience has two desktop entry points:

- **CodexAssistant**: a no-window silent launcher. Double-clicking it starts enhanced Codex directly.
- **CodexAssistant 管理工具**: a visible Tauri management app for install, uninstall, update, settings, logs, diagnostics, shortcut repair, and optional manual launch.
...
- Requiring the management tool to stay open while CodexAssistant is running.
...
- `codex-assistant-launcher`
  ...
  - Used by the `CodexAssistant` desktop entry point.

- `codex-assistant-tauri`
  - Visible management console.
  - Tauri commands call `codex-assistant-core`; no duplicated business logic.
...
The `CodexAssistant` desktop entry point is silent:
...
- Install and uninstall CodexAssistant entry points.
- Check for updates and perform updates.
- Edit backend settings.
- View latest launch status.
- Open logs and copy diagnostics.
- Repair or recreate the silent `CodexAssistant` shortcut.
- Optionally launch or repair a running CodexAssistant session.
...
1. User double-clicks `CodexAssistant`.
...
4. `codex-assistant-core` and `codex-assistant-data` execute the operation.
...
1. User opens `CodexAssistant 管理工具`.
...
- `CodexAssistant.lnk`: silent launcher, no management window.
- `CodexAssistant 管理工具.lnk`: opens the Tauri management console.
...
- `CodexAssistant.app`: silent launch wrapper.
- `CodexAssistant 管理工具.app`: visible management console.
