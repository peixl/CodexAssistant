...
<html lang="zh-CN">
  <head>
    ...
    <title>CodexAssistant 管理工具</title>
  </head>
  <body>
    <div id="app"></div>
...
```text
/Applications/CodexAssistant.app
/Applications/CodexAssistant 管理工具.app
```
...
```text
双击 CodexAssistant：静默启动增强版 Codex。
双击 CodexAssistant 管理工具：安装、卸载、更新、设置、日志和诊断。
```
...
- `CodexAssistant` shortcut starts Codex without opening management UI.
- Codex App shows the CodexAssistant injected menu.
- Delete/undo/export/move/settings routes work from the injected UI.
- `CodexAssistant 管理工具` opens and shows Overview status.
