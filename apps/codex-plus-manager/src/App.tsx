...
const SCRIPT_MARKET_REPOSITORY_URL = "https://github.com/peixl/CodexAssistantScriptMarket";
        ...
        <div className="brand">
          ...
          <div className="brand-copy">
            <div className="brand-title-row">
              <div className="brand-title">CodexAssistant</div>
              {hasUpdate ? (
                <button
            ...
            <div className="brand-subtitle">管理控制台 · IFQ.AI 出品</div>
          </div>
        </div>
        <nav className="nav">
          {routes.map((item) => {
...
function OverviewScreen({
  ...
  const historyProvider = relay?.configured ? "CodexAssistant" : "openai";
      ...
      <Panel className="hero-panel">
        <CardContent className="hero-content">
          <div className="hero-layout">
            <div>
              <div className="eyebrow">CodexAssistant 状态</div>
  ...
  return (
    <>
      <Panel>
        ...
        <CardContent>
          <div className="relay-grid">
            ...
            <Metric label="历史会话" value={relay?.configured ? "CodexAssistant" : "openai"} />
      ...
      <Panel>
        ...
        <CardContent>
          <div className="path-line loose">CodexAssistant 设置：{settings?.settings_path ?? "未加载设置文件。"}</div>
      ...
      <Panel>
        ...
        <CardContent>
          <label className="switch-row">
            ...
            />
            <span>
              <strong>启用 CodexAssistant 页面增强</strong>
    ...
    <>
        ...
        <CardContent>
          <label className="switch-row">
            ...
            <span>
              ...
              <small>开启后，通过 CodexAssistant 启动 Codex 前自动整理一次旧对话的归属标记。</small>
            </span>
          </label>
      ...
      <Panel>
        ...
        <CardContent>
          <GuideList
            items={[
              "自动修复只在 CodexAssistant 启动 Codex 前运行，不会常驻监控或反复改写。",
              "需要马上整理旧对话时，可以点击“立刻修复历史会话”。",
              "它不控制页面功能，也不影响 API URL 或 Key。",
              "切回官方时历史会话会整理为 openai；切到 API 时会整理为 CodexAssistant。",
            ]}
          />
        </CardContent>
      </Panel>
        ...
        <CardContent>
          <div className="recommend-hero">
            <div>
              ...
              <span>内容来自 peixl/Ad-List，分为赞助商推荐和普通推荐。</span>
            </div>
...
function MaintenanceScreen({
  ...
  return (
    <>
      ...
      <Panel>
        ...
        <CardContent>
          <label className="check-row">
            ...
            <span>卸载时移除 CodexAssistant 托管数据</span>
          </label>
          <Toolbar>
      ...
      <Panel>
        <CardHead title="自动接管" detail="Watcher 用于保持 CodexAssistant 接管状态" />
        <CardContent>
          <Toolbar>
      ...
      <Panel>
        ...
        <CardContent>
          ...
          <Toolbar>
            <Button onClick={() => void actions.launch()}>启动 CodexAssistant</Button>
...
function AboutScreen({
  ...
  return (
    <>
      <Panel>
        <CardHead title="关于 CodexAssistant" detail="由 IFQ.AI 出品 · 本地 Codex 增强、管理工具和安装包维护" />
        <CardContent>
          <div className="metric-list">
            <Metric label="CodexAssistant 版本" value={overview?.current_version ?? update?.currentVersion ?? "-"} />
            <Metric label="Codex 版本" value={overview?.codex_version ?? "未检测到"} />
            <Metric label="出品方" value="IFQ.AI" />
            <Metric label="项目地址" value="github.com/peixl/CodexAssistant" />
          </div>
          <Toolbar>
            <Button onClick={() => void actions.openExternalUrl("https://github.com/peixl/CodexAssistant")} variant="secondary">
              <ExternalLink className="h-4 w-4" />
              打开项目主页
            </Button>
            <Button onClick={() => void actions.openExternalUrl("https://github.com/peixl/CodexAssistant/issues")} variant="secondary">
              <ExternalLink className="h-4 w-4" />
              反馈问题
            </Button>
    ...
    <div className="relay-profile-editor">
      ...
      {showApiFields && profile.protocol === "chatCompletions" ? (
        <div className="hint-line relay-protocol-hint">
          <MessageCircle className="h-4 w-4" />
          <span>此上游会通过本地 127.0.0.1:57321 转成 Responses API，需要从 CodexAssistant 启动 Codex。</span>
        </div>
      ) : null}
      <div className="hint-line relay-protocol-hint">
        <ShieldCheck className="h-4 w-4" />
...
function healthItems(overview: OverviewResult | null, relay: RelayResult | null) {
  return [
    ...
    },
    {
      ...
      detail: overview?.silent_shortcut.path || "缺少 CodexAssistant 静默启动快捷方式时可在安装维护页修复。",
    },
    {
      title: "管理工具入口",
